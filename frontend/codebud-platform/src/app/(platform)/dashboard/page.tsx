...
export default function DashboardPage() {
    ...
    // Depend on stable primitives (not the `user` object, which gets a new
    // identity on every auth refresh) so these callbacks don't re-trigger
    // their effects and duplicate the fetches.
    ...
    const fetchData = useCallback(
        async (isRefresh = false) => {
            try {
                ...
                console.error("Failed to fetch aggregated dashboard:", err);
                // localStorage fallback
                if (typeof window !== "undefined") {
                    const stored = JSON.parse(
                        localStorage.getItem("all_submissions") || "[]",
                    );
                    const userSubs = stored.filter(
                        (s: any) =>
                            s.userId === userId ||
    ...
    useEffect(() => {
        if (!loading) {
            // Lazy load leaderboard data after main dashboard details are fetched
            fetchLeaderboardData();
        }
    }, [loading, fetchLeaderboardData]);
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                {activeTab === "mentorship" && (
                                    <MentorshipTab tasks={tasks} projects={projects} mentor={mentor} alerts={alerts} loading={loading} />
                                )}
...
function MentorshipTab({ tasks, projects, mentor, alerts, loading }: { tasks: any[], projects: any[], mentor: any, alerts: any[], loading: boolean }) {
    return (
        <motion.div
            key="mentorship"
        ...
        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <MyMentorWidget mentor={mentor} tasks={tasks} projects={projects} loading={loading} />
