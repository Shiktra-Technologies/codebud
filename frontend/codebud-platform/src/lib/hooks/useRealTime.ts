...
export function useRealTime(options: UseRealTimeOptions = {}): UseRealTimeReturn {
    // 30s default: each poll issues two full-collection fetches (/api/users +
    // /api/submissions); 3s polling exhausted the backend's per-route rate
    // limit within minutes and flooded the network tab.
    ...
    useEffect(() => {
        ...
        // Start polling if enabled; skip ticks while the tab is hidden.
