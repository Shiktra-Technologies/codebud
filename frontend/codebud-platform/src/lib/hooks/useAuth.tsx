...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    const startKeycloakLogin = useCallback(async (redirectUri?: string, state?: string): Promise<AuthResult> => {
        try {
            ...
            // Direct cross-origin call to the backend (proxy removed). The backend's
            // CORS allowlist must include this app's origin; credentials: 'include'
            // lets the backend set the oauth_intent cookie (same-site subdomains).
            const backendBase = (process.env.NEXT_PUBLIC_BACKEND_BASE_URL || 'http://localhost:5000').replace(/\/+$/, '');
    ...
    }), [user, userRole, loading, isAuthenticated, startKeycloakLogin, exchangeAuthorizationCode, refreshMe, logout, hasPermission]);
