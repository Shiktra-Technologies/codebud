...
export function GridCardSkeleton({ className }: { className?: string }) {
    return (
        <div
            className={cn(
                "rounded-xl border border-white/[0.06] bg-surface-2/40 p-5 space-y-4",
                className,
            )}
        >
            ...
            <Skeleton className="h-4 w-3/4 rounded bg-white/[0.05]" />
...
export function ListRowSkeleton({ className }: { className?: string }) {
    return (
        <div
            className={cn(
                "flex items-center gap-4 p-4 rounded-xl bg-surface-2/30 border border-white/[0.04]",
                className,
            )}
        >
            <Skeleton className="h-9 w-9 rounded-lg bg-white/[0.05] shrink-0" />
            <div className="flex-1 space-y-2">
                <Skeleton className="h-3.5 w-1/3 rounded bg-white/[0.05]" />
                <Skeleton className="h-2.5 w-1/5 rounded bg-white/[0.04]" />
            </div>
            <Skeleton className="h-5 w-14 rounded bg-white/[0.04] shrink-0" />
        </div>
    );
}
...
export function StatTileSkeleton({ className }: { className?: string }) {
    return (
        ...
        >
            ...
            <Skeleton className="h-6 w-12 rounded bg-white/[0.05]" />
        </div>
    );
}
