...
async function fetchBackendMe(request: NextRequest, token: string) {
    try {
        // Server-side call straight to the backend (no CORS applies here).
        const meUrl = `${BACKEND_BASE_URL}/api/me`;
...
export async function middleware(request: NextRequest) {
    ...
    // Link prefetches fire this middleware for every visible <Link>. Skip the
    // backend /api/me round-trip for them (JWT screening above still applies);
    // the real navigation will do the full check.
    const isPrefetch =
